...
function loadWidget(config) {
	...
	let userAction = false,
		userActionTimer,
		messageTimer,
		messageArray = ["好久不见", "你在干嘛", "看我干嘛", "动我干嘛", "我一直在。"];
	...
	(function registerEventListener() {
		...
		devtools.toString = () => {
			showMessage("你在偷看什么。", 6000, 9);
		};
		window.addEventListener("copy", () => {
			showMessage("复制什么了？", 6000, 9);
		});
		window.addEventListener("visibilitychange", () => {
			if (!document.hidden) showMessage("好久不见。", 6000, 9);
		});
	})();
	...
	(function welcomeMessage() {
		...
		if (location.pathname === "/") { // 如果是主页
			...
			if (location.hostname === referrer.hostname) text = `<span>「${document.title.split(" - ")[0]}」</span>想你~`;
